...
const createVideoCategory = async (
  data: VideoCategoriesDto
): Promise<ResponseObjectInterface> => {
  try {
    const response = await axiosInstance.post<ResponseObjectInterface>(
    ...
    return {
      ...
      errors: {
        message:
          axiosError.response?.data?.errors?.message || axiosError.message,
...
export const useVideoCategories = () => {
  ...
  const fetchVideoCategories = useCallback(async () => {
    try {
      const response = await axiosInstance.get<ResponseObjectInterface>(
        "/admin/video-categories"
      );
      return response.data;
    } catch (err) {
