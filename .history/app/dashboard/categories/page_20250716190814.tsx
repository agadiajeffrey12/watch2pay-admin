...
const CategoriesPage = () => {
  ...
  useEffect(()=>{
    if(data && data?.data?.videoCategories) {
      setCategories(data?.data?.videoCategories.map((cat: any, index: number) => ({
        id: index + 1, // Assuming the backend doesn't provide an ID
        name: cat.name,
        slug: cat.slug,
