...
// Alternative approach using source switching technique
const refreshSignedUrlAlternative = useCallback(async () => {
  if (!videoRef.current || !video || isRefreshing) return
  ...
  try {
    ...
    if (newStreamUrl && newStreamUrl !== streamUrl) {
      // Create a promise that resolves when we can seek to the current position
      const switchSource = new Promise<void>((resolve, reject) => {
        ...
        const handleError = (e: Event) => {
          ...
          reject(new Error('Video loading error during source switch'))
        }
        ...
        // Switch the source
        videoElement.src = newStreamUrl
      })

      try {
        // Perform the source switch
        await switchSource
        ...
        if (wasPlaying) {
          try {
            ...
            console.warn('Could not resume playback after refresh:', playError)
            setIsPlaying(false)
          }
        }
        
        console.log('Successfully refreshed URL with minimal interruption')
      } catch (switchError) {
        console.error('Source switch failed:', switchError)
        // The video will just continue with the old URL until next refresh
      }
    }
...
// Most robust approach: dual video elements (recommended)
const refreshSignedUrlDualVideo = useCallback(async () => {
  if (!videoRef.current || !video || isRefreshing) return
  ...
  try {
    ...
    const currentPosition = currentVideo.currentTime
    const currentVolume = currentVideo.volume
    const wasMuted = currentVideo.muted
    ...
    if (newStreamUrl && newStreamUrl !== streamUrl) {
      ...
      newVideo.addEventListener('play', () => setIsPlaying(true))
      newVideo.addEventListener('pause', () => setIsPlaying(false))
      newVideo.addEventListener('click', handlePlayPause)
      newVideo.preload = 'metadata'

      // Wait for the new video to be ready
      const readyPromise = new Promise<HTMLVideoElement>((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error('New video ready timeout'))
        }, 10000)

        const handleCanPlay = () => {
          clearTimeout(timeout)
          newVideo.removeEventListener('canplay', handleCanPlay)
          newVideo.removeEventListener('error', handleError)
          resolve(newVideo)
        }
        ...
        const handleError = () => {
          ...
          newVideo.removeEventListener('canplay', handleCanPlay)
          newVideo.removeEventListener('error', handleError)
          reject(new Error('New video loading failed'))
        }
      ...
      try {
        const readyVideo = await readyPromise
        
        // Replace the old video element with the new one
        currentVideo.parentNode?.replaceChild(readyVideo, currentVideo)
        ...
        console.log('Successfully replaced video element with new URL')
      } catch (error) {
        console.error('Failed to create new video element:', error)
        // Clean up the new video element
        newVideo.remove()
      }
    }
...
}, [video, streamUrl, refetch, isRefreshing, handleTimeUpdate, handleLoadedMetadata, handlePlayPause])

// Updated video error handler to work with the new refresh method
useEffect(() => {
  const videoElement = videoRef.current
  ...
  const handleVideoError = (e: Event) => {
    ...
    if (!isRefreshing) {
      ...
      refreshSignedUrlDualVideo()
    }
  }

  videoElement.addEventListener('error', handleVideoError)
  return () => videoElement.removeEventListener('error', handleVideoError)
}, [isRefreshing]) // Removed refreshSignedUrl dependency since we're using the dual video method

// Update the auto-refresh interval to use the dual video method
useEffect(() => {
  ...
  // Set up new interval for 3 minutes (180000 ms)
...
}, [video, streamUrl, refreshSignedUrlDualVideo])
