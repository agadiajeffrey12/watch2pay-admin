...
const CategoryForm: React.FC<CategoryFormProps> = ({ 
  ...
  // 1. Auto-focus the name input when component mounts
  const nameInputRef = useRef<HTMLInputElement>(null);
  
  // 2. Handle file input for image upload
    ...
    <div ref={formRef} className="space-y-4">
      ...
      <div className="flex items-center justify-center flex-col gap-3 border border-dashed py-4">
        ...
        <Button 
          ...
          className="px-6 py-3 bg-blue-600 rounded-md text-white"
        >
          Select
        </Button>
        {formData.image && (
          <img 
            src={formData.image} 
            alt="Preview" 
            className="mt-2 max-w-32 max-h-32 object-cover rounded-lg"
          />
        )}
      </div>
